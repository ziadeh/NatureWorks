...
import { StrapiLoginResponseT, StrapiUserT } from "@/types/strapi/User";
...
export const authOptions: NextAuthOptions = {
    ...
    CredentialsProvider({
      ...
      async authorize(credentials, req) {
        ...
        try {
          ...
          const uUser: StrapiUserT = await getCurrentUser(data.jwt);
          return {
  ...
  callbacks: {
      ...
      if (account) {
        if (account.provider === "google") {
          ...
          try {
            ...
            token.isJudge = strapiLoginResponse.user.isJudge;
            token.viewLastStep = strapiLoginResponse.user?.viewLastStep;
            token.viewPreviousStep = strapiLoginResponse.user?.viewPreviousStep;
            token.regions = strapiLoginResponse.user.regions;
          } catch (error) {
            throw error;
          }
        }
        if (account.provider === "credentials") {
          ...
          token.viewLastStep = user?.viewLastStep;
          token.viewPreviousStep = user?.viewPreviousStep;
    ...
    async session({ token, session }) {
      ...
      session.user.viewLastStep = token?.viewLastStep;
      session.user.viewPreviousStep = token?.viewPreviousStep;
